...
export default function FellowshipRoomRealtime({ roomSlug, initialMessages }: Props) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (inserted) {
        setMessages((prev) => {
          const exists = prev.some((msg) => msg.id === inserted.id);
          if (exists) return prev;
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
        ...
        <div style={{ display: "grid", gap: 18 }}>
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              <article
                key={msg.id}
                style={{ padding: "18px 18px 16px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}
              >
      ...
      <section style={{ display: "grid", gap: 12, padding: 20, borderRadius: 20, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <p style={{ opacity: 0.72, fontSize: "0.95rem", lineHeight: 1.7, margin: 0 }}>
