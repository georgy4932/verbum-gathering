...
function RoomStatus() {
  ...
  return (
    <p style={{ opacity: 0.68, margin: "0 0 14px", fontSize: "0.95rem" }}>
      {state === "connected"
        ? "You are present in the gathering."
        : `Connection: ${state}`}
    </p>
  );
}
...
function ParticipantList() {
  ...
  const names = Array.from(
    new Set(
      tracks
        .map((t) => t.participant?.name || t.participant?.identity)
        .filter(Boolean)
    )
  );
  ...
  if (!token) {
    return (
      ...
      >
        ...
        >
          <p style={{ opacity: 0.6, margin: "0 0 6px", fontSize: "0.92rem" }}>
            Enter gently
          </p>
        ...
        {error ? (
          <p style={{ color: "#fca5a5", marginTop: 14, marginBottom: 0 }}>
            {error}
          </p>
        ) : null}
      </div>
